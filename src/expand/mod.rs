...
impl FileExpand {
    pub fn expand_file<P: AsRef<Path>>(entrypoint: P) -> Result<String, Box<dyn Error + 'static>> {
        ...
        let plug = String::from("// This file was generated by https://github.com/viniciusth/komodo-rs\n");
        let mut final_file = plug + &this.parsed_files.remove(&entrypoint_path).unwrap();
        final_file.push('\n');
    ...
    fn parse_file<P: AsRef<Path>>(
        ...
        for line in f.lines() {
            if line.starts_with("use") {
                assert!(!adding, "use statement in use statement");
            ...
            if adding {
                use_stmts.last_mut().unwrap().push_str(line.trim());
